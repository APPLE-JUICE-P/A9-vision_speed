...
static bool Msg24_GPS_RAW_INT( uint8_t port , mavlink_message_t* msg_sd )
{
	...
	if( GetPositionSensor( default_rtk_sensor_index, &gps_sensor ) )
	{
		mavlink_msg_gps_raw_int_pack_chan( 
			...
			(gps_sensor.addition_inf[1]>1)?gps_sensor.addition_inf[1]:1 ,	//fixtype
			gps_sensor.position_Global.x*1e+7  ,	//lat
			gps_sensor.position_Global.y*1e+7  ,	//lon
			gps_sensor.position_Global.z*10 ,	//alt
			gps_sensor.addition_inf[4] ,	//eph
			gps_sensor.addition_inf[5] ,	//epv
			UINT16_MAX ,	//vel
			0 ,	//cog
			gps_sensor.addition_inf[0] ,	//satellites_visible
			0 ,	//alt_ellipsoid
			gps_sensor.addition_inf[4]*10 ,	//h_acc
			gps_sensor.addition_inf[5]*10 ,	//v_acc
			gps_sensor.addition_inf[6]*10 ,	//vel_acc
			0  //hdg_acc
		);		
		return true;
	}
...
static bool Msg124_GPS2_RAW( uint8_t port , mavlink_message_t* msg_sd )
{
	...
	if( GetPositionSensor( default_gps_sensor_index, &gps_sensor ) && GetPositionSensor( default_rtk_sensor_index, &rtk_sensor ) )
	{
		mavlink_msg_gps2_raw_pack_chan( 
			...
			gps_sensor.position_Global.y*1e+7  ,	//lon
			gps_sensor.position_Global.z*10 ,	//alt
			gps_sensor.addition_inf[4] ,	//h_acc
			gps_sensor.addition_inf[5] ,	//v_acc
			0 ,	//vel
			0 ,	//cog
			gps_sensor.addition_inf[0] ,	//satellites_visible
			0 ,	//dgps_numch
...
static bool Msg32_LOCAL_POSITION_NED( uint8_t port , mavlink_message_t* msg_sd )
{
  ...
  sprintf(str,"NED\r\n\r\n");
	Write_Uart1((uint8_t *)str, strlen(str), 1, 1);
	sprintf(str,"N:%5.3lf\r\n\r\n",Position.y);
	Write_Uart1((uint8_t *)str, strlen(str), 1, 1);
	sprintf(str,"E:%5.3lf\r\n\r\n",Position.x);
	Write_Uart1((uint8_t *)str, strlen(str), 1, 1);
	sprintf(str,"D:%5.3lf\r\n\r\n",-Position.z);
	Write_Uart1((uint8_t *)str, strlen(str), 1, 1);
    sprintf(str,"vN:%5.3lf\r\n\r\n",VelocityENU.y);
	Write_Uart1((uint8_t *)str, strlen(str), 1, 1);
	sprintf(str,"vE:%5.3lf\r\n\r\n",VelocityENU.x);
	Write_Uart1((uint8_t *)str, strlen(str), 1, 1);
	sprintf(str,"vD:%5.3lf\r\n\r\n",-VelocityENU.z);
	Write_Uart1((uint8_t *)str, strlen(str), 1, 1);
...
static bool Msg234_HIGH_LATENCY( uint8_t port , mavlink_message_t* msg_sd )
{
	...
	{
		gps_sat = gps_sensor.addition_inf[0];
		gps_fix = (gps_sensor.addition_inf[1]>1)?gps_sensor.addition_inf[1]:1;
		gps_lat = gps_sensor.position_Global.x*1e+7;
		gps_lon = gps_sensor.position_Global.y*1e+7;	
		gps_alt = gps_sensor.position_Global.z*10;
	}
